...
### Added
- **edit-time-tput.sh**: Отдельный скрипт для интерактивного редактирования времени через tput (29.12.2025)
  - Редактирование трёх полей: Intro Start, Intro End, Outro Duration (формат MM:SS)
  - Навигация: ↑/↓ между полями, ←/→ внутри поля
  - Визуальная индикация: желтый фон для курсора, зеленый для активного поля
  - Валидация: Intro End должен быть больше Intro Start
  - Возвращает результат в секундах через stdout: `intro_start|intro_end|outro`
  - Восстановление TTY после dialog (`exec < /dev/tty`, `stty sane`)
  - Тестовые скрипты: `Test/test_tput_simple.sh` для отладки
...
- **macOS: Стрелки не работали в tput редакторе (29.12.2025)**
  - Проблема: `read -t 0.5` не поддерживается на macOS bash (требуется целое число)
  - Решение: Изменен timeout с `0.5` на `1` в edit-time-tput.sh
  - Bash 3.x/4.x на macOS требует целочисленные значения для `-t`

- **Черный экран при вызове tput редактора из меню (29.12.2025)**
  - Проблема: После dialog окно не очищалось, TTY оставался в неправильном состоянии
  - Решение 1: Добавлен `clear` в serials.sh перед вызовом edit-time-tput.sh
  - Решение 2: Добавлено восстановление TTY в edit-time-tput.sh:
    ```bash
    exec < /dev/tty
    exec > /dev/tty
    stty sane
    ```
  - Решение 3: Изменен способ вызова - через временный файл вместо `$(...)` для сохранения TTY

### Known Issues
- ⚠️ **Времена не сохраняются в БД после редактирования через tput (29.12.2025)**
  - Симптомы: Редактор работает корректно, возвращает значения, но они не записываются в series_settings
  - Требуется отладка: проверить парсинг результата и вызов `save_settings_to_db`
  - Приоритет: Высокий
  - **СЛЕДУЮЩИЙ ШАГ:** Отладить сохранение в новой сессии

### Technical
- Протестировано на macOS (bash 5.x) и Raspberry Pi (bash 5.2.x)
- Стрелки работают на обеих платформах после исправления timeout
- Dialog overlay исправлен, интерфейс работает корректно
- Файлы добавлены: `edit-time-tput.sh`, `Test/test_tput_simple.sh`
- Файлы изменены: `serials.sh` (добавлена кнопка extra_button)
