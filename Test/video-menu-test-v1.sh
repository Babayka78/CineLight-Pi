...
show_menu() {
    ...
    # ВАРИАНТ 1: Добавляем строку статуса настроек в начало меню
    local settings_status=$(get_settings_status_menuitem "$current_dir")
    items+=("__SETTINGS__" "⚙ $settings_status")
    ...
    if [ $exit_code -eq 3 ]; then
        ...
        # ВАРИАНТ 1: Обработка выбора строки настроек
        if [ "$choice" == "__SETTINGS__" ]; then
            show_series_settings "$current_dir"
            # Вернуться в меню на строку настроек
            show_menu "$current_dir" "__SETTINGS__"
            return
        fi
