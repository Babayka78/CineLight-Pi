...
#            $4=series_prefix, $5=series_suffix
# Возвращает: exit code 0 (сохранено) или 1 (отменено)
# Сохраняет времена напрямую в БД
...
edit_all_times() {
    ...
    local intro_end="$2"
    local outro="$3"
    local series_prefix="$4"
    local series_suffix="$5"
    ...
    while true; do
        ...
        # Enter
            ...
            # Сохраняем в БД напрямую (series_prefix и series_suffix уже в параметрах функции)
            
            # Загружаем текущие настройки флагов из БД (чтобы не затереть их)
            local current_settings=$(db_get_series_settings "$series_prefix" "$series_suffix")
            local autoplay=0
            local skip_intro=0
            local skip_outro=0
            
            if [ -n "$current_settings" ]; then
                IFS='|' read -r autoplay skip_intro skip_outro _ _ _ <<< "$current_settings"
            fi
            
            # Сохраняем в БД (флаги сохраняются, обновляются только времена)
            db_save_series_settings "$series_prefix" "$series_suffix" $autoplay $skip_intro $skip_outro "$intro_start_sec" "$intro_end_sec" "$outro_sec"
            
            # Восстанавливаем терминал и выходим
            tput cnorm
            stty $old_tty
            clear
            exit 0
